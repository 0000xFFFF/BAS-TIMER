...
void infos_bas_init(void)
{
    ...
    if (!g_infos.bas.valid) {
        ...
        g_infos.bas.schedules[0].from = hms_to_today_seconds(1, 0, 0);     // 01:00:00
        g_infos.bas.schedules[0].to = g_infos.bas.schedules[0].from + hms_to_today_seconds(0, 15, 0); // + 15min
        g_infos.bas.schedules[0].duration = hms_to_today_seconds(0, 5, 0); // 5 min
        g_infos.bas.schedules[0].last_yday = -1;

        g_infos.bas.schedules[1].valid = true;
        g_infos.bas.schedules[1].from = hms_to_today_seconds(5, 0, 0);     // 05:00:00
        g_infos.bas.schedules[1].to = g_infos.bas.schedules[1].from + hms_to_today_seconds(0, 15, 0); // + 15min
        g_infos.bas.schedules[1].duration = hms_to_today_seconds(0, 5, 0); // 5 min
        g_infos.bas.schedules[1].last_yday = -1;
    }
