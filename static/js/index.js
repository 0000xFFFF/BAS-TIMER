...
function setup_button_auto_timer() {
    document.getElementById("btn_auto_timer").addEventListener("click", (event) => { event.preventDefault(); toggle_auto_timer(); });
}

function setup_button_auto_gas() {
    document.getElementById("btn_auto_gas").addEventListener("click", (event) => { event.preventDefault(); toggle_auto_gas(); });
}
...
embed_innerHTML("settings_time", "/fragments/settings_time.html").then(setup_settings_time);
embed_innerHTML("status_term", "/fragments/status_term.html").then(setup_term);
